...
export async function generateMetadata({ params }: CookiePolicyPageProps): Promise<Metadata> {
  ...
  const content = legalPackageContent.cookies[normalizeLocale(locale)];
...
export default async function CookiePolicyPage({ params }: CookiePolicyPageProps) {
  ...
  return <LegalPageLayout content={legalPackageContent.cookies[normalizedLocale]} locale={normalizedLocale} />;
}
