      ...
      <Table>
        ...
        <TableBody>
          {documents.length ? (
            documents.map((document) => (
              <TableRow key={document.id} className="border-slate-100">
                ...
                <TableCell className="max-w-80 text-slate-600">
                  {document.linkedSections.length
                    ? document.linkedSections
                        .map((section) => labels.modules[section] ?? section)
                        .join(", ")
                    : labels.notProvided}
                </TableCell>
            ...
            <TableRow>
              <TableCell colSpan={4} className="px-6 py-6 text-sm font-medium text-slate-500">
                {labels.noApprovedDocuments}
              </TableCell>
            </TableRow>
          )}
        </TableBody>
