...
## Faza 4.4.1 Forgot Password And Reset Password Flow

Use this checklist before inviting pilot suppliers.

Behavior:

- `/[locale]/login` includes a localized forgot-password link.
- `/[locale]/forgot-password` sends a Nhost password reset email with a localized redirect target at `/[locale]/reset-password`.
- The forgot-password success state is intentionally generic and does not reveal whether an account exists.
- `/[locale]/reset-password` accepts a Nhost `passwordReset:` ticket from the reset link when present, or an existing authenticated reset session when Nhost provides one.
- Reset password requires a new password and confirmation, checks the existing 9-character app policy, and redirects back to login after success.

Nhost configuration:

- Add the production reset URL to Nhost Auth allowed redirect URLs: `https://<production-domain>/en/reset-password`.
- Add the Croatian reset URL as well: `https://<production-domain>/hr/reset-password`.
- If preview deployments are used for QA, add their exact preview reset URLs before testing password reset emails.
- Keep Nhost Auth email templates pointed at the Nhost reset/verify link generated by `sendPasswordResetEmail`; the app handles the final redirect page.

Security notes:

- Do not log emails, reset tickets, passwords, auth response bodies, JWTs, cookies or raw Nhost errors.
- Do not show raw auth errors to users.
- Treat reset tickets as short-lived secrets. They should appear only in the reset link URL and in client memory during submission.

Manual QA:

1. Open `/hr/login` and click `Zaboravili ste lozinku?`.
2. Enter an email and confirm the generic success message appears.
3. Receive the Nhost reset email and open the reset link.
4. Set and confirm a new password on `/hr/reset-password`.
